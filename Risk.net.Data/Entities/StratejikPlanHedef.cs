...
{
    /// <summary>
    /// Veritabanýndaki StratejikPlanHedef tablosu ile yazýlým arasýnda iliþki kurmamýzý saðlayan kalýcý nesnedir
    /// </summary>
    public class StratejikPlanHedef : EntityBase, IEntity
    {
        ...
        public virtual string IsbirligiBirimAdlari
        {
            get
            {
                ...
                foreach (StratejikPlanIsbirligiBirim item in IsbirligiBirimler)
                {
                    if (sayac == 42 && item.Koordinatorluk?.Adi.IndexOf(" Ýl ") > -1)
                        continue;
