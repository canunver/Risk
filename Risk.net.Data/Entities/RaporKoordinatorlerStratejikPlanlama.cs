...
{
    /// <summary>
    /// Veritabanındaki RaporKoordinatorlerStratejikPlanlama view ile yazılım arasında ilişki kurmamızı sağlayan kalıcı nesnedir
    /// </summary>
    public class RaporKoordinatorlerStratejikPlanlama : RaporEntityBase, IEntity
    {
        ...
        public virtual List<StratejikPlanIzlemeDonem> Donemler
        {
            get
            {
                var donenDeger = new List<StratejikPlanIzlemeDonem>();
                    ...
                    if (!string.IsNullOrWhiteSpace(IzlemeDonemleri))
                    {
                        ...
                        foreach (var deger in d)
                        {
                            if (!string.IsNullOrWhiteSpace(deger))
                            {
                                ...
                                var izlemeDonem = new StratejikPlanIzlemeDonem();
                                izlemeDonem.Donem = Convert.ToInt32(degerler[0]);
                                izlemeDonem.PlanlananDeger = Convert.ToInt32(degerler[1]);
                                izlemeDonem.GerceklesenDeger = Convert.ToInt32(degerler[2]);
                                izlemeDonem.GerceklesenDegerYilSonu = Convert.ToInt32(degerler[3]);
