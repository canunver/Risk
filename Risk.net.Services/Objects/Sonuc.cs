...
{
    /// <summary>
    /// Entity verilerinin istemci katmanýna göndermek için kullanýlan deðiþken
    /// </summary>
    /// <remarks></remarks>
    public class Sonuc
    {
        ...
        {
            //Örnek: String or binary data would be truncated in table 'Risk.dbo.RiskEvreni', column 'AnahtarRiskGostergesiAdi'. Truncated value: 'yk sürecindeki proje sayýsý, ekip sayýsý, YK tamalanma süresinide içerir bir proje aðýrlýk oraný ile'.
            ...
            if (!string.IsNullOrWhiteSpace(deger))
            {
                if (deger.Contains("truncated"))
                {
                    try
                    {
                        ...
                        string mesaj = alanAdi + " alaný için en fazla " + alanBoyut + " karakterlik veri giriþi yapýlabilir!";
