...
{
    public class TanimSehirService : ITanimSehirService
    {
        private readonly IUnitOfWork<TanimSehir> _unitOfWork;
        private readonly IStringLocalizer<CustomResource> _sharedResource;

        public TanimSehirService(IUnitOfWork<TanimSehir> unitOfWork, IStringLocalizer<CustomResource> sharedResource)
        {
        ...
        public async Task<Sonuc> KayitGetirAsync(string kod)
        {
            ...
            try
            {
                var kayit = await _unitOfWork.KayitGetirAsync(c => c.Kod == kod, b => b.Ulke);
        ...
        {
            var selectData = await _unitOfWork.SorguHazirlaAsync(null, b => b.Ulke);

            return Arac.DataTablesJsonData(selectData, dataTablesParam);
        }

        public async Task<Sonuc> KaydetAsync(TanimSehir gelenNesne)
        {
            TanimSehir islemYapilan = new TanimSehir();
            ...
            try
            {
                ...
                else
                {
                    ...
                    eskiKayit.UlkeKod = gelenNesne.UlkeKod;
