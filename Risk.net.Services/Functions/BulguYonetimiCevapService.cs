...
{
    /// <summary>
    /// BulguYonetimiCevap iþlemlerinin yapýldýðý servis
    /// </summary>
    public class BulguYonetimiCevapService : IBulguYonetimiCevapService
    {
        /// <summary>
        /// IUnitOfWork<BulguYonetimiCevap> servisine ulaþmak için kullanýlan deðiþken
        /// </summary>
        /// <remarks></remarks>
        ...
        /// <summary>
        /// <see cref="Risk.net.Services.Functions.BulguYonetimiCevapService" /> 'ýn yeni bir örneðini baþlatan sýnýf
        /// </summary>
        ...
        public async Task<Sonuc> KaydetAsync(KullaniciDto kullanan, BulguYonetimiCevap gelenNesne)
        {
            ...
            //Cevap isteði bulgu baþýna tekken bu oluyordu. Bir bulgu için ayný kiþiye birden fazla kez gönderim yapýlabilir isteði geldi.
            //else if (!string.IsNullOrWhiteSpace(gelenNesne.BulguYonetimiKod) && !string.IsNullOrWhiteSpace(gelenNesne.IlgiliPersonelKod))//BulguKod ve Ilgili kiþi kod geldiyse
            ...
            try
            {
                ...
                //Onay verildiðinde mail gönderilecek
                //if (gelenNesne.CevapTarihi.HasValue)
            ...
            try
            {
                ...
                if (gelenNesne.Durum == (int)ENUMDurum.OnayaGonderdi || gelenNesne.Durum == (int)ENUMDurum.Onayli || gelenNesne.Durum == (int)ENUMDurum.GeriGonderildi || gelenNesne.Durum == (int)ENUMDurum.Reddedildi)
                {
                    ...
                    if (gelenNesne.Durum == (int)ENUMDurum.OnayaGonderdi)
                    {
                        ...
                        //Eðer onaylayan kiþi koordinatör ise kendisinin onaylayacak demektir. Otomatik onay yapýlsýn Melih 31.07.2023 
        ...
        public async Task<Sonuc> BilgiEPostaGonderAsync(KullaniciDto kullanan, string bulguKod, List<string> gelenNesne)
        {
            ...
            //Görevlendirme yapýlan bulgu bilgisini almak için
            Sonuc sonucBulgu = await _serviceBulguYonetimi.KayitGetirAsync(kullanan, bulguKod);
            if (!sonucBulgu.IslemSonuc || sonucBulgu.Nesne == null) return new Sonuc(ENUMIslemDurum.Uyari, "Bulgu bulunamadý");
            ...
            try
            {
                foreach (var item in gidecekler)
                {
                    ...
                    string mesaj = $"Sayýn {item.AdiSoyadi};<br>" +
                                   $"{denetimAdi} / {gidecekBulgu.Denetim.DenetimNo} nolu Denetim kapsamýnda " +
                                   $"'{gidecekBulgu.BulguTanimi}' bulgusu ile ilgili, sorumlusu olarak belirlendiðiniz Eylem için " +
                                   $"Risk Uygulamasýndan giriþ yaparak ilgili kýsýmlara {Arac.DateTimeToDDMMYYYY(gidecekBulgu.SonCevapTarihi)} tarihine kadar cevap vermeniz beklenmektedir.<br><br>" +
                                   $"Bilgilerinize önemle rica olunur.<br><br><br>" +
                                   $"Saygýlarýmýzla,<br>" +
                                   $"Ýç Denetim Koordinatörlüðü";

                    /*
                “Sayýn ……,
                . …Denetim Adý…. Denetimi kapsamýnda …Bulgu… bulgusu ile ilgili olarak,  sorumlusu olarak belirlendiðiniz Eylem için …. uygulamasýndan giriþ yaparak ilgili kýsýmlara ….. tarihine kadar cevap vermeniz beklenmektedir.
        ...
        {
            //Cevap verilen kayýt bilgisini almak için
            Sonuc sonucBulguCevap = await KayitGetirAsync(kullanan, bulguCevapKod);
            if (!sonucBulguCevap.IslemSonuc || sonucBulguCevap.Nesne == null) return new Sonuc(ENUMIslemDurum.Uyari, "Bulgu Cevap bulunamadý");
            ...
            if (!sonucDenetim.IslemSonuc || sonucDenetim.Nesne == null) return new Sonuc(ENUMIslemDurum.Uyari, "Denetim bulunamadý");
            ...
            try
            {
                ...
                string mesaj = $"'{gidecekBulgu.Denetim.DenetimNo}' nolu Denetim kapsamýnda " +
                               $"'{gidecekBulgu.BulguTanimi}' bulgusu {gidecekBulguCevap.IlgiliPersonel.AdiSoyadi} tarafýndan cevaplanmýþtýr.";
