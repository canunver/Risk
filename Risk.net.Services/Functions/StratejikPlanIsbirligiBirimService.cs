...
{
    /// <summary>
    /// StratejikPlanIsbirligiBirim iþlemlerinin yapýldýðý servis
    /// </summary>
    public class StratejikPlanIsbirligiBirimService : IStratejikPlanIsbirligiBirimService
    {
        /// <summary>
        /// IUnitOfWork<StratejikPlanIsbirligiBirim> servisine ulaþmak için kullanýlan deðiþken
        /// </summary>
        /// <remarks></remarks>
        private readonly IUnitOfWork<StratejikPlanIsbirligiBirim> _unitOfWork;
        /// <summary>
        ...
        /// <summary>
        /// <see cref="Risk.net.Services.Functions.StratejikPlanIsbirligiBirimService" /> 'ýn yeni bir örneðini baþlatan sýnýf
        /// </summary>
        /// <param name="unitOfWork"></param>
        /// <param name="sharedResource"></param>
        /// <remarks></remarks>
        public StratejikPlanIsbirligiBirimService(IUnitOfWork<StratejikPlanIsbirligiBirim> unitOfWork, IStringLocalizer<CustomResource> sharedResource)
        {
        ...
        /// Sonuc nesnesi döndürür
        /// </returns>
        public async Task<Sonuc> ListeleAsync(KullaniciDto kullanan, string stratejikPlanHedefKod)
        {
            var kayitlar = await _unitOfWork.ListeleAsync(a => a.StratejikPlanHedefKod == stratejikPlanHedefKod);
            if (kayitlar.Count > -1)
            {
        ...
        public async Task<object> TabloDoldurAsync(KullaniciDto kullanan, DataTablesParam dataTablesParam)
        {
            ...
            {
                StratejikPlanIsbirligiBirim aramaObj = Arac.DataTablesAramaNesne<StratejikPlanIsbirligiBirim>(new StratejikPlanIsbirligiBirim(), aramaDegeri);

                if (!string.IsNullOrWhiteSpace(aramaObj.StratejikPlanHedefKod))
                    selectData = await _unitOfWork.KosulEkleAsync(selectData, a => a.StratejikPlanHedefKod.Contains(aramaObj.StratejikPlanHedefKod));
            }
            else
            {
                selectData = await _unitOfWork.KosulEkleAsync(selectData, a => a.Birim.Adi.Contains(dataTablesParam.searchValue));
            }
        ...
        /// Sonuc nesnesi döndürür
        /// </returns>
        public async Task<Sonuc> KaydetAsync(KullaniciDto kullanan, StratejikPlanIsbirligiBirim gelenNesne)
        {
            StratejikPlanIsbirligiBirim islemYapilan = new StratejikPlanIsbirligiBirim();
        ...
        /// Sonuc nesnesi döndürür
        /// </returns>
        public async Task<Sonuc> SilAsync(KullaniciDto kullanan, string stratejikPlanHedefKod)
        {
            string hata = "";

            if (string.IsNullOrWhiteSpace(stratejikPlanHedefKod))
                hata = "<li>" + _sharedResource["Kontrol.Duzenle.KodAlaniBos"] + "</li>";

            try
            {
                await _unitOfWork.SilAsync(d => d.StratejikPlanHedefKod == stratejikPlanHedefKod);
                await _unitOfWork.KaydetAsync();
            }
            catch (System.Exception ex)
            {
