...
{
    /// <summary>
    /// RiskAzaltmaPlaniIliski iþlemlerinin yapýldýðý servis
    /// </summary>
    public class RiskAzaltmaPlaniIliskiService : IRiskAzaltmaPlaniIliskiService
    {
        /// <summary>
        /// IUnitOfWork<RiskAzaltmaPlaniIliski> servisine ulaþmak için kullanýlan deðiþken
        /// </summary>
        /// <remarks></remarks>
        private readonly IUnitOfWork<RiskAzaltmaPlaniIliski> _unitOfWork;
        /// <summary>
        ...
        /// <summary>
        /// <see cref="Risk.net.Services.Functions.RiskAzaltmaPlaniIliskiService" /> 'ýn yeni bir örneðini baþlatan sýnýf
        /// </summary>
        /// <param name="unitOfWork"></param>
        /// <param name="sharedResource"></param>
        /// <remarks></remarks>
        public RiskAzaltmaPlaniIliskiService(IUnitOfWork<RiskAzaltmaPlaniIliski> unitOfWork, IStringLocalizer<CustomResource> sharedResource)
        {
        ...
        {
            var kayitlar = await _unitOfWork.ListeleAsync(a => a.RiskAzaltmaPlaniKod == riskAzaltmaPlaniKod, o => o.IliskiKod);
            if (kayitlar.Count > -1)
            {
        ...
        public async Task<object> TabloDoldurAsync(KullaniciDto kullanan, DataTablesParam dataTablesParam)
        {
            ...
            {
                RiskAzaltmaPlaniIliski aramaObj = Arac.DataTablesAramaNesne<RiskAzaltmaPlaniIliski>(new RiskAzaltmaPlaniIliski(), aramaDegeri);
                ...
                if (!string.IsNullOrWhiteSpace(aramaObj.IliskiKod))
                    selectData = await _unitOfWork.KosulEkleAsync(selectData, a => a.IliskiKod.Contains(aramaObj.IliskiKod));
            }
            else
            {
                selectData = await _unitOfWork.KosulEkleAsync(selectData, a => a.IliskiKod.Contains(dataTablesParam.searchValue));
            }
        ...
        /// Sonuc nesnesi döndürür
        /// </returns>
        public async Task<Sonuc> KaydetAsync(KullaniciDto kullanan, RiskAzaltmaPlaniIliski gelenNesne)
        {
            RiskAzaltmaPlaniIliski islemYapilan = new RiskAzaltmaPlaniIliski();

            string hata = "";

            if (string.IsNullOrWhiteSpace(gelenNesne.RiskAzaltmaPlaniKod))
                hata += "<li>" + _sharedResource["Kontrol.Duzenle.RiskAzaltmaPlaniKodAlaniBos"] + "</li>";
            if (string.IsNullOrWhiteSpace(gelenNesne.IliskiKod))
                hata += "<li>" + _sharedResource["Kontrol.Duzenle.RiskAzaltmaPlaniIliskiKodAlaniBos"] + "</li>";
            ...
            try
            {
                ...
                else
                {
                    ...
                    eskiKayit.IliskiKod = gelenNesne.IliskiKod;
